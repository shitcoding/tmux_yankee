...
// escFlushDelay is how long the event loop waits for the rest of a fragmented
// escape sequence before flushing a lone ESC. 25ms is imperceptible for exiting
// visual/search yet long enough to catch a same-burst continuation.
// ponytail: fixed 25ms heuristic; raise it if split sequences slip through over
// high-latency links (this is the vim ttimeoutlen knob).
...
func (t *TUI) Run() error {
	...
	//
	// Escape-flush debounce: when a read ends part-way through an escape
	// sequence (a fragment split across reads, e.g. an SGR mouse sequence cut by
	// a TCP segment boundary over SSH), wait a short quiet gap for the rest
	// before flushing a lone ESC, so the trailing bytes aren't misparsed as keys.
	//
	// Both debounces select on the timer's OWN channel, with a nil channel to
	// disable the case while unarmed. Go 1.23+ makes Stop/Reset race-free — no
	// stale tick is delivered after Stop/Reset returns — so a superseded timer
	// can never trigger a spurious render or flush. No side channel, goroutine,
	// or drain needed.
	var resizeTimer, escFlushTimer *time.Timer
	var resizeTimerC, escFlushTimerC <-chan time.Time
	defer func() {
		if resizeTimer != nil {
			resizeTimer.Stop()
		}
		if escFlushTimer != nil {
			escFlushTimer.Stop()
		}
	}()
	...
	for {
		...
		case event := <-inputCh:
			...
			// Resolve parser state left pending after this read.
			//
			// If an escape sequence is only part-way through (a fragment split
			// across reads), (re)arm the escape-flush timer and wait for the rest
			// before flushing — otherwise a lone ESC flush would strand the
			// trailing bytes to be misparsed as literal keys. Re-arming Stops the
			// old timer first, so a superseded deadline can't fire (Go 1.23+).
			//
			// Otherwise flush now: this resolves a standalone ESC (kept
			// responsive, no second keypress needed) or a deferred command, and
			// disarms the timer.
			if t.parser.PendingEscape() {
				if escFlushTimer == nil {
					escFlushTimer = time.NewTimer(escFlushDelay)
				} else {
					escFlushTimer.Stop()
					escFlushTimer.Reset(escFlushDelay)
				}
				escFlushTimerC = escFlushTimer.C
			} else {
				if escFlushTimer != nil {
					escFlushTimer.Stop()
				}
		...
		case <-resizeCh:
			// Debounce: reset the timer on each SIGWINCH so we re-render only
			// after 50ms of quiet. Stop before Reset so a superseded deadline
			// can't fire (Go 1.23+ guarantees no stale tick after Stop).
			if resizeTimer == nil {
				resizeTimer = time.NewTimer(50 * time.Millisecond)
			} else {
				resizeTimer.Stop()
				resizeTimer.Reset(50 * time.Millisecond)
			}
			resizeTimerC = resizeTimer.C
		...
		case <-escFlushTimerC:
			// Escape-flush debounce elapsed with a sequence still incomplete:
			// resolve it now (standalone ESC, or discard an incomplete CSI).
			// Guarded by PendingEscape in case a continuation read raced in first.
			escFlushTimerC = nil
			if t.parser.PendingEscape() {
				if flushCmd := t.parser.Flush(); flushCmd.Type != input.CommandNone {
					if t.handleCommand(flushCmd) {
